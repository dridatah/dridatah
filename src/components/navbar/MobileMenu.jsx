  ...
  return (
    <div ref={mobileMenuRef} className={`mobile-menu ${isMobileMenuOpen && 'active'}`}>
      ...
      <Link href="/" className="logo">
        <Image src="/assets/images/logo-white.png" width={140} height={40} alt="logo" className="img-fluid" />
      </Link>
      <ul className="mobile-nav-menu">
        {menuData.map((menuItem, index) => (
          <li className={`has-submenu ${activeSubMenu === index && 'active'}`} key={index}>
            <Link
              href={menuItem.link}
              onClick={(e) => {
            ...
            {menuItem.submenu && (
              <i className={`fas fa-angle-down ${activeSubMenu === index && 'icon-rotate'}`}></i>
            )}
            {menuItem.submenu && (
              <ul className={`submenu-wrapper ${activeSubMenu === index && 'd-block'}`}>
